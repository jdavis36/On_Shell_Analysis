...
def main(argv):
    ...
    if opt == '-h':
      ...
      sys.exit()
        ...
        for name in split_names:
          inputtrees.append(name)
    elif opt in ("-o", "--ofile"):
        outputdir = arg
    elif opt in ("-cf", "--config"):
        configname = arg
  if not all([inputtrees, outputdir, configname]):
        print('Collect_Templates.py -i <treelistpath> -o <output_directory> --config <config_file>')
        sys.exit(2)
  if not outputdir.endswith("/"):
        outputdir = outputdir+"/"
  Input_Trees = inputtrees
  print(Input_Trees)
  ...
  def separate_by_year(Input_File):
    Files_By_Year = {"2016APV":[],"2016":[],"2017":[],"2018":[],"Run2":[]}
  ...
  for fin in Input_Trees:
          ...
          for fs in Final_States:
            ...
            for year in files_by_year.keys():
              if files_by_year[year] != [] and year in Years:
                ...
                config_string = " --config "+configname
                ...
                template_cmd = cmd + input_string + production_mode_string + category_string + final_state_string + year_string + out_string + config_string
                print(template_cmd)
    ...
    elif "DATA" in fin.upper():
      SubFolder_Name = "Data_Templates/"
      files_by_year = separate_by_year(fin)
      input_string = " -i " + fin
      production_mode_string = " -p Data "
      condor_dir = ""
      if not os.path.exists(outputdir+"condor_Data"):
        os.mkdir(outputdir+"condor_Data")
      condor_dir = outputdir+"condor_Data/"
      for category in Event_Categories:
          category_string = " -c " + category
          for fs in Final_States:
            final_state_string = " -f " + fs
