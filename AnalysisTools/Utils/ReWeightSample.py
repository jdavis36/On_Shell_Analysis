...
def GetIntFromTemplateName_MCFM(IntTemplateName,ProductionMode,isData):
  ...
  if SumOfOrders == 2:
    ...
    # We need special considerations for gammaH samples since it can only be produced through Zy or yy couplings
    if Use_Decay_Only: # This should only allow couplings of order 2 
      if any(order != 2 for order in Couplingdict[Coupling]):
        raise ValueError("Invalid order for decay only")
      Pure_Couplings = []
      Mixed_Couplings = []
      for coupling in CouplingDict:
        if 'gammaH' in ProductionMode:
          coupl_string = coupling+"_"+CouplingValuesDict[coupling]
          Pure_Couplings.append("p_Gen_Dec_SIG_"+coupl_string+"_JHUGen")
      sorted_couplings = Sort_Name_Order(CouplingDict.keys())
  raise ValueError("Invalid Iso Template Name")



def GetIntFromTemplateName_JHUGen(IntTemplateName,ProductionMode,isData,Analysis_Config):
  CouplingDict={} #Stores Coupling Orders  
  CouplingValuesDict=GetCouplingValues()
  Use_Decay_Only = False
  Use_Prod_Only = False
  if "Decay_Only" in Analysis_Config.Hypothesis_List["Options"] :
    Use_Decay_Only = True
  if "Prod_Only" in Analysis_Config.Hypothesis_List["Options"] :
    Use_Prod_Only = True
  if "FixDecaygammaH" in Analysis_Config.Hypothesis_List["Options"]:
    Do_SM_Decay = True
  print("IntTemplate",IntTemplateName)
  #sort out order of each coupling#
  if re.search("(g1(?P<Order>[1234]))",IntTemplateName):
  ...
  if SumOfOrders == 2:
    ...
    if "ggH" in ProductionMode:
      ...
      raise ValueError("Note that any other production mode must have higher order coupling specified")
    # Now make the correct combination of coupling strings # 
    return Mixed_Couplings[0] +"-"+Pure_Couplings[0]+"-"+Pure_Couplings[1]
  elif SumOfOrders == 4: 
    # We need special considerations for gammaH samples since it can only be produced through Zy or yy couplings
    if Use_Decay_Only: # This should only allow couplings of order 2 
      if any(CouplingDict[Coupling] != "2" for Coupling in CouplingDict.keys()):
        raise ValueError("Invalid order for decay only")
      Pure_Couplings = []
      Mixed_Couplings = []
      for coupling in CouplingDict:
      ...
      for coupling in sorted_couplings:
        coupl_string += coupling+"_"+CouplingValuesDict[coupling]+"_"
      Mixed_Couplings.append("p_Gen_Dec_SIG_"+coupl_string+"JHUGen")
    elif Use_Prod_Only: # This should only allow couplings of order 2 
      if any(CouplingDict[Coupling] != "2" for Coupling in CouplingDict.keys()):
        raise ValueError("Invalid order for decay only")
      Pure_Couplings = []
      Mixed_Couplings = []
      coupl_string = "" # Reset the Coupling String 
      for coupling in CouplingDict:
        if 'gammaH' in ProductionMode:
          coupl_string = coupling+"_"+CouplingValuesDict[coupling]
          Pure_Couplings.append("p_Gen_GammaH_SIG_"+coupl_string+"_JHUGen")
      sorted_couplings = Sort_Name_Order(CouplingDict.keys())
      print("Sorted_Couplings: ", sorted_couplings)
      coupl_string = "" # Reset the Coupling String again
      for coupling in sorted_couplings:
        coupl_string += coupling+"_"+CouplingValuesDict[coupling]+"_"
      Mixed_Couplings.append("p_Gen_GammaH_SIG_"+coupl_string+"JHUGen")
      # Need to force the decay to look like SM #
      if Do_SM_Decay:
        SM_Decay_String = "p_Gen_Dec_SIG_ghz1_1_JHUGen"
        return "("+Mixed_Couplings[0] +"-"+Pure_Couplings[0]+"-"+Pure_Couplings[1]+")"+"*"+SM_Decay_String
      else:
        return Mixed_Couplings[0] +"-"+Pure_Couplings[0]+"-"+Pure_Couplings[1]
    else:
      if 'gammaH' in ProductionMode:
...
def GetPureFromTemplateName_JHUGen(OutputHypothesis,ProductionMode,isData,Analysis_Config):
  prodName = ''
  coupl_string = ''
  coupling_name=GetIsoHVVCouplingName(OutputHypothesis)
  CouplingValuesDict=GetCouplingValues()
  ...
  if "Prod_Only" in Analysis_Config.Hypothesis_List["Options"]:
     Use_Prod_Only = True
  if "FixDecaygammaH" in Analysis_Config.Hypothesis_List["Options"] and 'gammaH' in ProductionMode:
     Use_Prod_Only = True
     Do_SM_Decay = True
  if not isData:
    if 'gg' in ProductionMode:
      ...
      Generator =  "JHUGen"
      return "p_Gen_"+prodName+"_SIG_"+coupl_string+"_"+Generator
    elif "qqH" in ProductionMode:
      if Use_Decay_Only:
        coupl_string=coupling_name+"_"+CouplingValuesDict[coupling_name]
        Generator = "JHUGen"
        return "p_Gen_Dec_SIG_"+coupl_string+"_"+Generator
      else:
        raise ValueError("Prod + Decay not supported")
    elif "WminusH" in ProductionMode:
      if Use_Decay_Only:
        coupl_string=coupling_name+"_"+CouplingValuesDict[coupling_name]
        Generator = "JHUGen"
        return "p_Gen_Dec_SIG_"+coupl_string+"_"+Generator
      ...
      if Use_Decay_Only:
        ...
        raise ValueError("Prod + Decay not supported")
    elif 'gammaH' in ProductionMode:
      prodName = 'GammaH'
      decName = 'Dec'
      Generator = "JHUGen"
      if Use_Decay_Only: # This should only allow couplings of order 2
        coupl_string = coupling_name+"_"+CouplingValuesDict[coupling_name]
        return "p_Gen_Dec_SIG_"+coupl_string+"_JHUGen"
      elif Use_Prod_Only: # This should only allow couplings of order 2
        coupl_string = coupling_name+"_"+CouplingValuesDict[coupling_name]
        if Do_SM_Decay:
          SM_Decay_String="p_Gen_Dec_SIG_ghz1_1_JHUGen"
          return "p_Gen_GammaH_SIG_"+coupl_string+"_JHUGen"+"*"+SM_Decay_String
        else:
          return "p_Gen_GammaH_SIG_"+coupl_string+"_JHUGen"
      else:
        coupl_string_prod = coupling_name+"_"+CouplingValuesDict[coupling_name]
        coupl_string_dec = coupling_name+"_"+CouplingValuesDict[coupling_name]
        if Do_SM_Decay:
          coupl_string_dec = "ghz1_1"
        return "p_Gen_"+decName+"_SIG_"+coupl_string_dec+"_"+Generator + "*" + "p_Gen_"+prodName+"_SIG_"+coupl_string_prod+"_"+Generator
...
def GetConstantsAndMELA_From_Template_Name_JHUGen(Template_Name,ProductionMode,isData,Analysis_Config):
  Branch_Names_String = ''  
  ...
  if (grouped_names["HVVpure"] != None):
    Branch_Names_String = GetPureFromTemplateName_JHUGen(grouped_names["HVVpure"],ProductionMode,isData,Analysis_Config)
  if (grouped_names["HVVint"] != None):
    Branch_Names_String = GetIntFromTemplateName_JHUGen(grouped_names["HVVint"],ProductionMode,isData,Analysis_Config)
...
def Reweight_Branch_NoHff_From_Template_Name(InputTree,template_name,isData,Analysis_Config,lumi,year,DoMassFilter,TopTreeName):
  ...
  Grouped = re.match("(?P<production>gg|tt|bb|qq|qqgamma|Z|Wplus|Wminus|V|VBF|gamma)H_(?:(?P<Hffpure>0(?:PM|M)ff)_)?(?:(?P<HVVpure>0(?:PM|M|PH|L1|L1Zg|Mgg|PHgg|MZg|PHZg|wCuu|wCdd|wCss|wCcc))|(?P<HVVint>(?:g(?:1|2|4|1prime2|hzgs1prime2|2gg|4gg|2Zg|4Zg)[1234])*))$",template_name)
  if Grouped != None:
    grouped_names = Grouped.groupdict()
    ...
    OutputHypothesis = "bkg"
  elif "data_obs" in template_name:
    OutputHypothesis = "data_obs"
  else:
