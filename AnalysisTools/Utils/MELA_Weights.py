...
def parse_prob(probability):
  ...
  parsed_dict={"Process":None,"ProdMode":None,"MatrixElement":None,"coupl_dict":{},"isReco":None,"Prod":None,"Dec":None,"JES":None,"JEC":None,"JER":None,"HbbTree":None}
  ...
  if "gg" in probability:
  ...
  elif "zh" in probability:
    if "jhugen" in probability:
      parsed_dict["Prod"] = True
      parsed_dict["Dec"] = False
      parsed_dict["ProdMode"] = "HadZH"
    else:
      raise ValueError("Choose correct Matrix element for ZH in production probability")
  ...
  elif "wh" in probability:
    if "jhugen" in probability:
      parsed_dict["Prod"] = True
      parsed_dict["Dec"] = False
      parsed_dict["ProdMode"] = "HadWH"
    else:
      raise ValueError("Choose correct Matrix element for WH in probability")
    ...
    parsed_dict["Prod"] = True
    parsed_dict["Dec"] = False
  elif "vbf" in probability:
    parsed_dict["ProdMode"] = "JJVBF"
    if "jhugen" in probability:
      parsed_dict["Prod"] = True
      parsed_dict["Dec"] = False
    else:
      raise ValueError("Choose correct Matrix element for VBF prodcution probability")
    ...
    parsed_dict["Prod"] = True
    parsed_dict["Dec"] = False
  elif "dec" in probability:
    parsed_dict["ProdMode"] = "ZZINDEPENDENT"
    parsed_dict["Prod"] = False
    parsed_dict["Dec"] = True
  elif "gammah" in probability:
    parsed_dict["ProdMode"] = "GammaH"
    parsed_dict["Prod"] = True
    parsed_dict["Dec"] = False
  # Sort MatrixElement #
  if "jhugen" in probability:
  ...
  # Add Temporary Flag for Matej #
  if "hbb" in probability:
    parsed_dict["HbbTree"] = True
  # Sort Couplings #
  ...
  try:
    ...
    for i, entry in enumerate(tqdm.tqdm(t, desc="Events Processed for " + infile, total=t.GetEntries()), start=1):
      ...
      for prob in probdict:
        ...
        if parsed_prob_dict["HbbTree"]: 
          # Note that we use the same setup purely for simplicity. There are no lepton four vectors because we do not care about decay Hbb
          #leptons = SimpleParticleCollection_t(SimpleParticle_t(pid, tlv(pt, eta, phi, 0)) for pid, pt, eta, phi in zip([11,-11,13,-13], [t.bVec.Pt(),0,t.bbarVec.Pt(),0], [t.bVec.Eta(),0,t.bbarVec.Eta(),0], [t.bVec.Phi(),0,t.bbarVec.Phi(),0]))
          leptons = SimpleParticleCollection_t(SimpleParticle_t(pid, tlv(pt, eta, phi, m)) for pid, pt, eta, phi ,m in zip([5,-5,], [t.bVec.Pt(),t.bbarVec.Pt()], [t.bVec.Eta(),t.bbarVec.Eta()], [t.bVec.Phi(),t.bbarVec.Phi()], [t.bVec.M(),t.bbarVec.M()]))
          # The jets in this case will be the photons
          jets = SimpleParticleCollection_t([SimpleParticle_t(22, tlv(t.gammaVec.Pt(), t.gammaVec.Eta(), t.gammaVec.Phi(), t.gammaVec.M()))])
          # Will use the b quarks as the mothers in this case. To reconstruct Higgs Pt etc
          #mothers = SimpleParticleCollection_t(SimpleParticle_t(23, tlv(pt, eta, phi, m)) for pt, eta, phi, m in zip([t.bVec.Pt(),t.bbarVec.Pt()],[t.bVec.Eta(),t.bbarVec.Eta()],[t.bVec.Phi(),t.bbarVec.Phi()],[t.bVec.M(),t.bbarVec.M()]))
          mothers = 0
          m.setInputEvent(leptons, jets, mothers, 1)
        elif parsed_prob_dict["isReco"]:
