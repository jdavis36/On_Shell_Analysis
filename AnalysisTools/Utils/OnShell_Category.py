...
def Return_Untagged_Only(): # Only for SM gammaH production
...
def Tag_Untagged_and_gammaH(PhotonPt,PhotonIsCutBasedLooseID):
  ...
  #Need to check the object for any high Pt photon above the cut
  if PhotonPt is not None:
    if len(PhotonPt) != 0:
      for i in range(len(PhotonPt)):
        if (PhotonPt[i] > Photon_Pt_Cut) and (PhotonIsCutBasedLooseID[i] == True):  
          return gammaHTagged
  return Untagged
  # This will mimic the old categorization 
  #if(len(PhotonIsCutBasedLooseID)!=0 and PhotonPt[0]>Photon_Pt_Cut):
  #  return gammaHTagged
  #else:
  #  return Untagged

def Tag_Untagged_and_qq_gammaH(PhotonPt,PhotonIsCutBasedTightID): # Only for SM gammaH production
  ...
  if PhotonPt is not None:
    if len(PhotonPt) != 0:
      for i in range(len(PhotonPt)):
        if (PhotonPt[i] > Photon_Pt_Cut) and (PhotonIsCutBasedTightID[i] == True):  
          return gammaHTagged
  return Untagged
