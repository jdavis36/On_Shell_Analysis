...
def Unroll_1D_OnShell(hist):
    xbins = hist.GetNbinsX()
    
    temp_pos = TH1F("temp_pos","",xbins,0,xbins)
    temp_neg = TH1F("temp_neg","dif",xbins,0,xbins)
    ...
    if (has_negative and ( "bkg" in tnname or "Data" in tnname  or "0PH" in tnname or "0PM" in tnname or "L1" in tnname or "0M" in tnname or "0w" in tnname)):
      for x in range (1,xbins+1):
              binx_c = hist.GetXaxis().GetBinCenter(x)
              ibin =  hist.FindBin(binx_c)
              cont  = hist.GetBinContent(ibin)
    ...
    elif (has_negative or not ( "bkg" in tnname or "Data" in tnname  or "0PH" in tnname or "0PM" in tnname or "L1" in tnname or "0M" in tnname or "0w" in tnname) ):
