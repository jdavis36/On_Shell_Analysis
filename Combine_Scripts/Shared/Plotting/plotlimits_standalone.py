...
if __name__ == "__main__":
    ...
    parser.add_argument("--CMStext", default="")
    ...
    parser.add_argument("--higgsbasis", nargs=1, type=str, action="append")
    parser.add_argument("--squareaxis", nargs=1, type=str, action="append")
...
def plotlimits(*scans, **kwargs):
    ...
    for scan in scans:
        ...
        for filename in scan.filenames:
            ...
            for entry in islice(t, startfrom, None):
                ...
                sinw = sin2W**(1/2.)
                cosw = (1-sin2W)**(1/2.)
                e = (4*math.pi*alpha_qed)**(1/2.)
                if higgsbasis != None:
                    if "cyy" == higgsbasis[0][0]:
                        fa3 = -(2./e**2) * fa3
                    elif "cyyt" == higgsbasis[0][0]:
                        fa3 = -(2./e**2) * fa3
                    elif "czy" == higgsbasis[0][0]:
                        fa3 = -((2*sinw*cosw)/e**2) * fa3
                    elif "czyt" == higgsbasis[0][0]:
                        fa3 = -((2*sinw*cosw)/e**2) * fa3
                    elif higgsbasis != None:
                        raise ValueError("Wrong higgs basis string")
                if squareaxis != None:           
                  if squareaxis[0][0].upper() == "TRUE":
                    fa3 = fa3**2
                if 2*deltaNLL > 100 : 
                    continue
    ...
    style.CMS(CMStext, lumi=luminosity, lumitext=lumitext, drawCMS=drawCMS,  extratextsize=0.035)

    if letter is not None: style.subfig(letter, *letterposition, textsize=lettersize)

    drawlines(CLtextposition, xmin=xmin, xmax=xmax, textsize=lettersize)
...
def drawlines(xpostext="left", xmin=-1, xmax=1, logscale=False, xsize=.1, ysize=.03, yshift=0, yshift68=None, yshift95=None, textsize=None, PRL=False, CLbelow=False, arbitraryparameter=None):
    ...
    if yshift68 is None: yshift68 = yshift + 0.015
    if yshift95 is None: yshift95 = yshift + 0.015
